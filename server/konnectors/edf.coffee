...
fetchListerContratClientParticulier = (requiredFields, entries, data, callback) ->
    ...
    edfRequestPost path, body, (err, result) ->
        ...
        try
            ...
            client.telMobile = getF bpObject, 'tns:Coordonnees', 'tns:NumTelMobile'
            ...
            contracts = contratElems.map (contratElem) ->
                ...
                switch contract.energie
                    when 'Électricité'
                        ...
                        contract.structureTarifaire = getF offreSouscriteObj, 'tns:StructureTarifaire'

                    when 'Gaz'
                        contract.optionPrix = getF offreSouscriteObj, 'tns:OptionPrix'
                ...
                if cadranElem
                    ...
                    compteur.type = getF contratElem, 'tns:DonneesTechniques', 'tns:TypeCompteur'

                    contract.compteur = compteur

                    contract.consommationAnnuelle = getF cadranElem, 'tns:ConsommationAnnuelle'

                contract.horrairesHeureCreuses = getF contratElem, 'tns:DonneesTechniques', 'tns:HorrairesHC'

                releveElem = getF contratElem, 'tns:Releve'

                if releveElem
                    releve = {}
                    releve.prochaineReleve = getF releveElem, 'tns:ProchaineDateReleveReelle'
                    releve.saisieReleveConfiance = getF releveElem, 'tns:SaisieRC'
                    releve.dateFermetureReleveConfiance = getF releveElem, 'tns:DateFermetureRC'
                    releve.prochaineDateOuvertureReleveConfiance = getF releveElem, 'tns:ProchaineDateOuvertureRC'
                    releve.prochaineDateFermetureReleveConfiance = getF releveElem, 'tns:ProchaineDateFermetureRC'
                    releve.prochaineDateFermetureReelle = getF releveElem, 'tns:ProchaineDateFermetureReelle'
                    releve.saisieSuiviConso = getF releveElem, 'tns:SaisieSC'
                    releve.prochaineDateOuvertureSaisieConso = getF releveElem, 'tns:ProchaineDateOuvertureSC'

                    contract.releve = releve

                contract.services = []
                if contratElem['tns:ServicesSouscrits']
                    contract.services = contract.services.concat contratElem['tns:ServicesSouscrits'].map (serviceElem) ->
                ...
                if resBody['tns:ServicesSouscrits']
                    contract.services = contract.services.concat resBody['tns:ServicesSouscrits'].map (serviceElem) ->

                        service =
                            nom: getF serviceElem, 'tns:nomService'
                            start: getF serviceElem, 'tns:dateSouscription' # TODO : to UTC
...
fetchVisualiserAccordCommercial = (requiredFields, entries, data, callback) ->
    ...
    edfRequestPost path, body, (err, result) ->
        ...
        try
            ...
            paymentTerms.typeEncaissement = getF acoElem, 'ns:detail', 'ns:modeEncaissement'
            paymentTerms.modifIbanAutorisee = getF acoElem, 'ns:detail', 'ns:modifIbanAutorisee'
            ...
            paymentTerms.periodiciteFacture = getF acoElem, 'ns:facturation', \
                                    'ns:periodicite'
            paymentTerms.dateProchaineFacture = getF acoElem, 'ns:facturation', \
                                      'ns:dateProchaineFacture'
...
fetchRecupereDocumentContractuelListx = (requiredFields, entries, data, callback) ->
...
fetchEdeliaProfile = (requiredFields, entries, data, callback) ->
    ...
    getEdelia data.edeliaToken, '/sites/-/profiles/simple?ts=' + new Date().toISOString(), (err, response, obj) ->
        error = null
        try
...
fetchEdeliaMonthlyElecConsumptions = (requiredFields, entries, data, callback) ->
    ...
    getEdelia data.edeliaToken, "/sites/-/monthly-elec-consumptions?begin-month=2012-01&end-month=#{moment().add(1, 'month').format('YYYY-MM')}&ended=false", (err, response, obj) ->
        ...
        try
            ...
            entries.consumptionStatements = statements.concat entries.consumptionStatements or []
...
fetchEdeliaSimilarHomeYearlyElecComparisions = (requiredFields, entries, data, callback) ->
    return callback() if data.noEdelia or data.noElec

    K.logger.info "fetchEdeliaSimilarHomeYearlyElecComparisions"
    getEdelia data.edeliaToken, "/sites/-/similar-home-yearly-elec-comparisons?begin-year=2012", (err, response, objs) ->
        error = null
        try
            if response.statusCode is 404 or response.statusCode is 500
                ...
                throw null
            if err
                K.logger.error 'Wihle fetchEdeliaSimilarHomeYearlyElecComparisions'
                K.logger.error err
                throw err
...
fetchEdeliaElecIndexes = (requiredFields, entries, data, callback) ->
    ...
    getEdelia data.edeliaToken, "/sites/-/elec-indexes?begin-date=2012-01-01&end-date=#{moment().format('YYYY-MM-DD')}&types=", (err, response, objs) ->
...
fetchEdeliaMonthlyGasConsumptions = (requiredFields, entries, data, callback) ->
    ...
    getEdelia data.edeliaToken, "/sites/-/monthly-gas-consumptions?begin-month=2012-01&end-month=#{moment().add(1, 'month').format('YYYY-MM')}&ended=false", (err, response, obj) ->
...
fetchEdeliaSimilarHomeYearlyGasComparisions = (requiredFields, entries, data, callback) ->
    return callback() if data.noEdelia or data.noGas

    K.logger.info "fetchEdeliaSimilarHomeYearlyGasComparisions"
    getEdelia data.edeliaToken, "/sites/-/similar-home-yearly-gas-comparisons?begin-year=2012", (err, response, objs) ->
        ...
        try
            ...
            if err
                K.logger.error 'Wihle fetchEdeliaSimilarHomeYearlyGasComparisions'
                K.logger.error err
                throw err
...
fetchEdeliaGasIndexes = (requiredFields, entries, data, callback) ->
    ...
    getEdelia data.edeliaToken, "/sites/-/gas-indexes?begin-date=2012-01-01&end-date=#{moment().format('YYYY-MM-DD')}&types=", (err, response, objs) ->
...
updateOrCreateDocs = (requiredFields, entries, data, next) ->
    async.series [
        ...
        (cb) ->
            updateOrCreate [entries.paymentTerms], ['vendor', 'clientId'], PaymentTerms, cb

        (cb) -> updateOrCreate entries.homes, ['pdl'], Home, cb
        (cb) ->
            updateOrCreate entries.consumptionStatements
            , ['contractNumber', 'statementType', 'statementReason', 'statementCategory', 'start'],
...
saveMissingBills = (requiredFields, entries, data, callback) ->
    Bill.all (err, bills) ->
        async.eachSeries bills, (bill, cb) ->
            ...
            fetchPDF data.edfToken, entries.client, bill.number, (err, base64String) ->
...
K = module.exports = require('../lib/base_konnector').createNew
    ...
    fetchOperations: [
        ...
        #    data.edfToken = "AQIC5wM2LY4SfcyLlaVwgBxANPtrsPQvgiVOGu3TVruRZd8.*AAJTSQACMDIAAlNLABMzMTYzMTQ3NjAxODY3MTUxNzQ0AAJTMQACMDE.*"
        #    callback()
        getEDFToken
        ...
        #(requiredFields, entries, data, callback) ->
        #    data.edeliaToken = "d684e244-7826-4229-87f1-3e9f8c434806"
        #    data.contract =
        #        pdl: ""
        #        number: "123478"
        #    callback()
